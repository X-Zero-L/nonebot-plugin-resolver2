...
对[nonebot-plugin-resolver2](https://github.com/fllesser/nonebot-plugin-resolver2)进行了一点修改，主要是 youtube 部分的解析结果增强，包括自动识别横竖视频获取对应封面，不过对 youtube 的解析似乎应该加点违禁词过滤器？（我猜作者就是基于这个考虑才只返回了视频标题 😀）

[nonebot-plugin-resolver](https://github.com/zhiyu1998/nonebot-plugin-resolver) 重制版

| 平台    | 触发的消息形态                         | 视频 | 图集 | 音频 |
| ------- | -------------------------------------- | ---- | ---- | ---- |
| B 站    | BV 号/链接(包含短链,BV,av)/卡片/小程序 | ✅​  | ✅​  | ✅​  |
...
|        r_bili_ck         |  否  |   ""   |                          B 站 cookie, 可不填，若填写，必须含有 SESSDATA 项，可附加 B 站 AI 总结功能,如果需要长期使用此凭据则不应该在**浏览器登录账户**导致 Cookies 被刷新，建议注册个小号获取 cookie                           |
|         r_ytb_ck         |  否  |   ""   |                                                                                      Youtube cookie, Youtube 视频因人机检测下载失败，需填                                                                                      |
|         r_proxy          |  否  |  None  |                                                                                                   youtube, tiktok, x 的代理                                                                                                    |
|      r_need_upload       |  否  | False  |                                                                                                  音频解析，是否需要上传群文件                                                                                                  |
|      r_need_forward      |  否  |  True  |                                                               **不超过** 4 条的解析消息是否需要合并转发，配置为 False ，超过 4 条的解析消息仍然会用合并转发包裹                                                                |
...
| r_video_duration_maximum |  否  |  480   |                                                                                                B 站视频最大解析时长，单位：_秒_                                                                                                |
