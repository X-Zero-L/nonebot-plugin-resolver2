...
@bilibili.handle()
@handle_exception()
async def _(event: MessageEvent, keyword: str = Keyword(), searched: re.Match[str] = KeyPatternMatched()):
    if not is_not_in_disabled_groups_by_bilibili(event):
        return
...
async def _(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    if not is_not_in_disabled_groups_by_bilibili(event):
        await bili_music.finish("本群已关闭哔哩哔哩解析")
