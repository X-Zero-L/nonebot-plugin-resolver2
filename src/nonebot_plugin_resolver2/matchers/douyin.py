...
from .filter import is_not_in_disabled_groups_by_douyin
...
@douyin.handle()
@handle_exception()
async def _(event: MessageEvent, searched: re.Match[str] = KeyPatternMatched()):
    if not is_not_in_disabled_groups_by_douyin(event):
        return
